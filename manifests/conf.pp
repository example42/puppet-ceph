...
# = Define: ceph::conf
#
# With this define you can manage any ceph configuration file
#
# == Parameters
#
# [*template*]
...
#   Example: template => 'site/ceph/my.conf.erb',
#
# [*content*]
...
#   Example: source => 'puppet:///site/ceph/my.conf',
#
# [*ensure*]
...
define ceph::conf (
  ...
  $manage_path    = pickx($path, "${ceph::config_dir_path}/${name}")
  $manage_content = default_content($content, $template)
  $manage_mode    = pickx($mode, $ceph::config_file_mode)
  $manage_owner   = pickx($owner, $ceph::config_file_owner)
  $manage_group   = pickx($group, $ceph::config_file_group)
  $manage_require = pickx($config_file_require, $ceph::config_file_require)
  $manage_notify  = $config_file_notify ? {
    'class_default' => $ceph::manage_config_file_notify,
    default         => $config_file_notify,
  }

  file { "ceph_conf_${name}":
    ensure  => $ensure,
    source  => $source,
